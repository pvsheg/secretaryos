  ...
  try {
    ...
    if (customTemplate) {
      ...
      if (existsSync(templateSrc)) {
        ...
        writeFileSync(templateDest, getDefaultTemplate(isPaid), 'utf-8')
      }
    }
...
function getDefaultTemplate(isPaid: boolean = false): string {
  const headerLeft = isPaid
    ? `[]`
